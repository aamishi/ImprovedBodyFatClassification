...
#| tbl-cap: "BMI Categories as per the Government of Canada Guidelines"
...
The BodyM Data Set, referred to as the Body Measurements data set throughout this paper, was collected by Ruiz et al. [@Ruiz2022] with a focus on underrepresented body types in the estimation of fat and its subsequent health risks. The primary data captured in this collection consists of the front and lateral silhouettes of 2,779 test subjects. These silhouettes were then converted into black-and-white images for use in their augmentation model. The body measurements in this data set were generated using their Adversarial Body Simulator (ABS), which was specifically designed to capture underrepresented body types. The S3 package  included three data sets: Training, Test A, and Test B. For the purposes of this paper, only the Training data set is used. The visual images of the test subjects were photographed and 3D-scanned by lab technicians.
...
### Measurement of Data
Medical records obtained through medical–nutritional consultations of BiomediKcal–Advanced Medical Nutrition & Lifestyle Center of Barranquilla were used to construct this data set. Height in centimeters and fat mass values were obtained using the Tanita MC-780MA medical equipment.
...
BMI is a crude way of classifying people. Athletes are especially at risk at being mis-classified despite a lower body mass weight but a higher muscle mass. 

## Weaknesses and next steps {#sec-weaknesses-point}

Neither of the dataset include children and cannot be applied to anyone under the age of 18. It is important to consider age as a primary factor while diagnosing any medical condition. Additionally, fat deposits affect different age demographics differently. For example, an obese child can develop early symptoms of adult obesity if not corrected in the younger years. Likewise, menopausal women face hormonal changes that cause increased abdominal obesity (@Davis2012). Information such as pregnancy status should also be considered if WHR is to be introduced as a replacement for BMI in clinical settings.

My models are recorded for ethically different groups of people. While the AWS BodyM Data Set is collected for a varied group of ethnicity, the Body Composition data set is specifically collected only in Barranquilla, Columbia. The way humans across different races store and respond to excess fat is also largely different [@Deurenberg1998]. Hence, the models should have accounted for potential confounders that could arise due to enthicity. Furthermore, the models does not consider the difference in data sizes. While I did use proportions as much as possible, the data sets are skewed in slightly different directions. Both datasets have low representation for Underweight (by BMI) people. More analysis can be done in this regard to adjust for the lack of representation. The above factors make the data sets not precisely comparable.

A suitable next step for this research paper would be to include age and other demographic details such as lifestyle category. Additional information about a person's lifestyle could further improve the diagnosis of any obesity related disease. It would also be significant to include whether the specific ethnic information of a person or to use data set from the same geographical location. 

Lastly, the models can improve the proxies for `fat_percentage_category` by studying current literature on the relationship between BMI and fat percentage. The model bases the categories based on the existances of similar trends without taking into consideration any adjustment factors. 
\newpage
...
Both models were considerd due to their high accuracy which is shown in [@tbl-accuracy]. Inclusion of weight and fat free mass, respectively, caused a higher accuracy but I suspect it is because of over-fitting of data. 
...
#| message: false
#| label: tbl-accuracy
#| tbl-cap: "Accuracy of Models"
# Predicted probabilities for each category
accuracy_1 <- mean(cleaned_data_model$predicted_category == cleaned_data_model$fat_percentage_category)

accuracy_2 <- mean(body_mass_model$predicted_category == body_mass_model$fat_percentage_category)

accuracy_table <- data.frame(
  Model = c("Measurements Model", "Body Mass Model"),
  Accuracy = c(accuracy_1*100, accuracy_2*100)
)

# Generate a table in Quarto
...
To establish a more causational relationship between anthropocentric measurements and actual fat mass to indicate obesity, we can set up an interview based study. This would include interviewing the test subjects about their lifestyle, their fitness levels, additional hormonal fluctuations such as pregnancy or menopause. This would provide us more information to be able to differentiate between the circumference measurement. We could also interview the subjects about their ethnicity, age and if they currently have any metabolism enhancing or reducing treatments. 

This interview would provide the base for a more detailed comparative analysis of the establishment of bodily measurements and its correlation with body fat. The same set of people can be recorded for their anthropocentric measurements as well as body composition measurement. It would then make the results of the study more directly application to conclude a relationship.
