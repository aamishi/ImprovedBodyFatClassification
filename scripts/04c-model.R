...
ggplot(body_mass_read) +
  geom_boxplot(aes(x = bmi_category, y = fm, fill = "Actual"), alpha = 0.5, position = "dodge") +
  geom_boxplot(aes(x = bmi_category, y = predicted_fm, fill = "Estimated"), alpha = 0.5, position = "dodge") +
  labs(x = "BMI Category", y = "Fat Percentage",
       title = "Boxplot of Actual vs Estimated Fat Percentage") +
  scale_fill_manual(values = c("Actual" = "blue", "Estimated" = "orange")) +
  theme_minimal()
